...
// Function to fetch package details from npm registry
export async function getNpmPackageDetails(packageName) {
  ...
  try {
    ...
    const cachedData = apiCache.get(cacheKey);
    if (cachedData && (Date.now() - cachedData.timestamp) < CACHE_EXPIRY) {
      console.log(`Using cached package details for ${packageName}`);
      return cachedData.data;
    }
    
    const apiUrl = `https://registry.npmjs.org/${encodeURIComponent(packageName)}`;
    
    console.log(`Fetching npm package details from: ${apiUrl}`);
    
    const response = await fetch(apiUrl);
    
    if (!response.ok) {
      console.error(`Error fetching npm package details for ${packageName}: ${response.status} ${response.statusText}`);
      return null;
    }
    ...
    const packageDetails = {
      ...
      lastUpdate: data.time?.modified || data.time?.created,
      maintainers: data.maintainers,
      keywords: data.keywords,
      installCommand: `npm install ${data.name}`
    };
    
    // Clean up repository URL if it's a git URL
    if (packageDetails.repository && packageDetails.repository.startsWith('git+')) {
      packageDetails.repository = packageDetails.repository
        .replace('git+', '')
        .replace('.git', '');
    }
    ...
    console.error(`Error fetching npm package details for ${packageName}:`, error);
    return null;
  }
}
